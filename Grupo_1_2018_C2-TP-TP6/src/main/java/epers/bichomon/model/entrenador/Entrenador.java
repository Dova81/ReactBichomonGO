...
public class Entrenador {
    ...
    public boolean contains(Bicho bicho) {
        return bichos.stream().anyMatch(bicho::equals);
    }

    public void abandonar(Bicho bicho) {
        if (!contains(bicho))
            throw new BichoIncorrectoException(bicho.getID());
        if (this.bichos.size() <= 1)
            throw new CantidadBichosInsuficienteException(this.bichos.size());
    ...
    public ResultadoCombate duelo(Bicho b) {
        if (!contains(b))
            throw new BichoIncorrectoException(b.getID());
